...
| F10 | **Double toast notifications** | `error.interceptor.ts` + all components | Error interceptor shows a toast on every non-401 error, then re-throws. Components that also have `error:` callbacks show a second toast. ~10 affected locations. | Either remove per-component toasts and let the interceptor handle it, or add an `HttpContext` token to skip interceptor toast for specific requests. |
...
| F12 | **SignalR not stopped on 401 logout** | `error.interceptor.ts` | When interceptor detects 401 and calls `auth.logout()`, `signalR.stop()` is never called (only called from navbar logout). Connection keeps running with invalid token. | Call `signalR.stop()` inside `auth.logout()`. |
...
| F53 | **Medium** | ~~**Customer deletion cascade-deletes all order history**~~ | ~~`AppDbContext` FK config~~ | ✅ **FIXED** — Changed Customer→Orders FK from `DeleteBehavior.Cascade` to `DeleteBehavior.Restrict` in both `CustomerConfiguration.cs` and `OrderConfiguration.cs` (dual definition). `CustomerService.DeleteAsync()` now loads orders via `.Include()` and throws `InvalidOperationException` with clear message if customer has orders ("Cannot delete customer with N order(s). Order history is preserved for accounting."). Controller catches and returns `409 Conflict`. Cart items and chat messages still cascade-delete (transient data). Migration `RestrictCustomerOrderDeletion` created. | ~~Restrict deletion~~ ✅ Done |
...
| F72 | **Low** | ~~**Customers: `loadCounts()` silently swallows errors**~~ | ~~`customers.component.ts`~~ | ✅ **FIXED** — Added error handler that sets `subscriberCount` and `totalCount` to `-1` on API failure. Template uses conditional rendering: shows "N/A" when counts are negative instead of misleading zeros. Error interceptor still shows the toast for the underlying API failure. | ~~Show N/A on error~~ ✅ Done |
