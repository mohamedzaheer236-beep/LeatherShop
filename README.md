...
| **UI Polish (Zero `::ng-deep`)** | ✅ All PrimeNG overrides in global `styles.scss`. Zero `::ng-deep` in any component — all 3 previous usages (toast, chat search, product dropdown) moved to global rules with `body .p-*` prefix. Comprehensive overrides for navbar, toolbar, card, table, tag, button, dropdown, input, dialog, checkbox, progress spinner. Design system: indigo accent (#6366f1), dark navbar (#1a1a2e), gold brand (#e0c097), Inter font. |
...
| F3 | **WhatsApp webhook signature not validated** | `WhatsAppWebhookController.cs` | *(Duplicate of C4)* — Meta sends `X-Hub-Signature-256` header on every POST. The controller never validates it. Attackers can POST fabricated messages to create fake orders. | Validate `X-Hub-Signature-256` using HMAC-SHA256 with your App Secret. |
| F4 | **Race condition: overselling during checkout** | `ChatBotService.cs` `PlaceOrder()` | *(Duplicate of H1)* — Stock checked then decremented without DB-level locking. Two concurrent checkouts can both pass the stock check and oversell. | Use `UPDATE Products SET StockQuantity = StockQuantity - @qty WHERE Id = @id AND StockQuantity >= @qty` and check affected rows, or add a `RowVersion` concurrency token. |
...
| F21 | **`.ToLower()` kills DB indexes** | `CustomerService.cs`, `ProductService.cs`, `ChatService.cs` | *(Duplicate of M3 / P11)* — `.ToLower().Contains()` generates `LOWER()` SQL, preventing PostgreSQL from using indexes. | Use `EF.Functions.ILike()` for case-insensitive search on Npgsql. |
| F22 | **`PaginatedResult.TotalPages` divide-by-zero** | `PaginatedResult.cs` | *(Duplicate of P5)* — `TotalCount / (double)PageSize` when `PageSize = 0` throws. Only `OrdersController` clamps `pageSize >= 1`; other callers don't validate. | Guard: `PageSize <= 0 ? 0 : (int)Math.Ceiling(...)`. |
...
| F48 | **High** | ~~**Product Description MaxLength still mismatched in Fluent API**~~ | ~~`ProductConfiguration.cs` L18-19~~ | ✅ **FIXED** — Changed `.HasMaxLength(1000)` to `.HasMaxLength(2000)` in `ProductConfiguration.cs`. Model attribute, DTO, and Fluent API all now consistently use 2000. DB column will update on next migration. See H5 (also marked fixed). | ~~Align Fluent API to 2000~~ ✅ Done |
