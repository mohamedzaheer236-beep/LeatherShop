...
| F10 | **Double toast notifications** | `error.interceptor.ts` + all components | Error interceptor shows a toast on every non-401 error, then re-throws. Components that also have `error:` callbacks show a second toast. ~10 affected locations. **PARTIALLY FIXED — `deleteCustomer()` component toast removed; interceptor handles it.** | Either remove per-component toasts and let the interceptor handle it, or add an `HttpContext` token to skip interceptor toast for specific requests. |
